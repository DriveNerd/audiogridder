...
#if AG_ENABLE_CUDA
    bool useGPU = shouldUseGPUProcessor(id);
    
    if (useGPU) {
        logln("Creating GPU processor for plugin: " << id);
        proc = std::make_shared<GPUProcessor>(*this, id, getSampleRate(), getBlockSize());
    } else {
        proc = std::make_shared<Processor>(*this, id, getSampleRate(), getBlockSize());
    }
#else
    // GPU disabled - use regular processor
    proc = std::make_shared<Processor>(*this, id, getSampleRate(), getBlockSize());
#endif
...
bool ProcessorChain::shouldUseGPUProcessor(const String& id) const {
#if AG_ENABLE_CUDA
...
#else
    // GPU disabled at compile time
    return false;
#endif
    
    // For now, enable GPU processing for all plugins
    // In a production version, you might want to have a whitelist/blacklist
    // or configuration options to control which plugins use GPU
    
    // You could add logic here to:
    // - Check plugin type (VST3, VST2, AU)
    // - Check plugin name/manufacturer
    // - Check configuration settings
    // - Check available GPU memory
    
    return true;  // Enable GPU for all plugins when available
}
